<div class="p-4">
  <a href="{{ route('shop.index') }}" class="text-brand-accent-light hover:text-gray-700 transition-colors">&larr; Back to Shop</a>
    ...
    <div>
              ...
              <div class="flex items-center justify-between p-4 border border-brand-accent-light rounded-xl {{ $selectedVariantId === $v->id ? 'ring-2 ring-brand-accent-light bg-brand-accent-light/10' : 'hover:bg-gray-50' }} transition-all">
                ...
                <div>
                  <button type="button" wire:click="selectVariant({{ $v->id }})"
                          class="bg-brand-accent-light flex gap-2 justify-center rounded-lg p-2 hover:border hover:border-brand-accent-light hover:bg-transparent cursor-pointer text-sm font-medium {{ $selectedVariantId === $v->id ? 'ring-2 ring-brand-accent-light' : '' }}">
      ...
      <div class="flex items-center gap-4 mt-8">
        ...
        <button type="button" wire:click="addToCart"
                class="bg-brand-accent-light flex gap-2 justify-center rounded-lg p-3 hover:border hover:border-brand-accent-light hover:bg-transparent cursor-pointer font-medium {{ !$selectedVariantId ? 'opacity-50 cursor-not-allowed' : '' }}"

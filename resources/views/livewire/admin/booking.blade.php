<div>
    ...
    <div class="">
        ...
        {{-- Action Zone --}}
        {{-- Table Zone --}}
        <x-ui.admin-table title="Bookings" :subtitle="count($booking).' total'">
            <x-slot name="head">
                <tr>
            ...
            @if (count($booking) > 0)
                @foreach ($booking as $key => $row)
                    <tr>
                        ...
                        <td class="px-6 py-4">{{ $row->client->name ?? '' }}</td>
                        <td class="px-6 py-4">
                            <div class="flex gap-3">
                                <div class="">{{ $row->code_booking }}</div>
                                <div class="flex items-center">
                                ...
                                <div @mouseenter="open = true" @click="open = !open">
                                    ...
                                    <div x-show="open" class="absolute z-40 w-auto mt-2 bg-white rounded-md shadow-lg -left-20 ring-1 ring-black ring-opacity-5" @mouseenter="open = true" @click.away="open = false" @mouseleave="open = false">
                                        <div class="flex flex-col py-1">
                                            ...
                                            @else
                                                <button class="w-full px-4 py-2 text-sm text-left text-gray-700 hover:bg-gray-100" wire:click="toggleStatus({{ $row->id }})" wire:confirm='Are you sure want to disable / cancel this booking ?'>Disable</button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <td colspan='6' class="px-6 py-4">
                            <div class="flex gap-10 ">
                                ...
                                <div class="flex flex-col ">
                                    <div class="">
                                        <p class="">Total Payment (After Deposit ${{ $row->deposit_price_booking }})</p>
                                        <p class="text-xl font-semibold">$ {{ $row->total_price_booking - $row->deposit_price_booking }}</p>
                                    </div>
                                    <div class="">
                                ...
                                <div class="min-w-48 w-fit">
                                    ...
                                    <ul>
                                        @foreach ($row->detailService as $serv)
                                            <li class="text-lg">
                                                <p class="font-semibold text-wrap">({{ $serv->service->category->name_service_categori }}) {{ $serv->name_service }}</p>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                                        ...
                                        <div class="flex flex-col w-full gap-3">
                                            <div class="">
                                                @if ($row->status !== 'completed')
                                                    <div class="">
                                                        <x-pages.btn type="success" value="Complete" class="w-full" wire:click="completeBooking({{ $row->id }})" wire:confirm="Are you sure you want to complete this booking status?" wire:loading.attr="disabled" wire:target="completeBooking">
                                                            <span wire:loading.remove wire:target="completeBooking">Complete</span>
                                                            <span wire:loading wire:target="completeBooking">Loading...</span>
                                                        </x-pages.btn>
                                                    </div>
                                                @else
                                                    <div class="">
                                                        <x-pages.btn type='danger' value='Uncomplete' wire:click='completeBooking({{ $row->id }})' wire:confirm="Are you sure you want to  revoke this booking status?" />
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="">
                                                @if ($row->is_deposit_paid == false)
                                                    <div class="">
                                                        <x-pages.btn type="success" value="Verify Deposit" wire:click="confirmDepositPayment({{ $row->id }})" wire:confirm="Are you sure you want to verify this deposit status?" wire:loading.attr="disabled" wire:target="confirmDepositPayment">
            ...
            @else
                <tr>
                    <td colspan='6' class="px-6 py-8 text-center">No data found</td>
                </tr>
            @endif
        </x-ui.admin-table>
        {{-- Table Zone --}}

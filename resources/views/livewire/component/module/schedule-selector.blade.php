        ...
        <div class="">
            <x-pages.inputs.select wire:model.live='selectedDate' wire:click='$refresh'>
                ...
                @foreach ($dataBookingDate as $index => $item)
                    <option value="{{ $item->id }}">{{ Carbon\Carbon::parse($item->date_schedule)->format('d-m-Y') }}
                    </option>
                @endforeach
        ...
        @for ($i = 0; $i < count($dataBookingDate); $i++)
            <div class="grid grid-cols-3 gap-2 my-3 {{ $dataBookingDate[$i]->id == $selectedDate ? '' : 'hidden' }}">
                @foreach ($dataBookingDate[$i]->times as $key => $bookingTime)
                    @php
                        $inputId = 'timeSlot-' . str_replace(':', '-', str_replace(' ', '-', $key)) . $i;
                    @endphp
                    <div class="">
                        <label for="{{ $inputId }}"
                            ...
                            <input wire:model.live='timeSelected' {{ $bookingTime->is_book == true ? 'disabled' : '' }}
                                type="radio" id="{{ $inputId }}" name="timeSlot" value="{{ $bookingTime->id }}"
                                class="mr-2">

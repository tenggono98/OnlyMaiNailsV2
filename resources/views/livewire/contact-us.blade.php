<div class="max-w-2xl mx-auto content-flow px-5" data-aos-skip>
    ...
    <form wire:submit.prevent="submit" class="bg-white rounded-xl border border-[#fadde1] p-6 ux-card">
        <div class="grid grid-cols-1 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1">Name</label>
                <input type="text" wire:model.defer="name" class="w-full p-3 border border-[#fadde1] rounded-lg form-control" placeholder="Your name" />
                @error('name') <small class="text-red-600">{{ $message }}</small> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Email</label>
                <input type="email" wire:model.defer="email" class="w-full p-3 border border-[#fadde1] rounded-lg form-control" placeholder="you@example.com" />
                @error('email') <small class="text-red-600">{{ $message }}</small> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Message</label>
                <textarea wire:model.defer="message" rows="6" class="w-full p-3 border border-[#fadde1] rounded-lg form-control resize-y" placeholder="How can we help you?"></textarea>
                @error('message') <small class="text-red-600">{{ $message }}</small> @enderror
            </div>
            <div class="flex items-center justify-between gap-3">
                <div class="">
                    <button type="submit" class="ux-btn bg-[#fadde1] px-6 py-3 rounded-lg font-medium">
                        Send Message
                    </button>
                </div>
                <div class="">

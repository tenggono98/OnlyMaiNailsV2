<div>
    ...
    <div class="grid grid-cols-1 gap-4 mx-auto lg:grid-cols-3 lg:px-10">
            ...
            <form>
                <div class="my-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="w-full form-control" placeholder="8 Digit Code" name="codeSearch"
                            id="codeSearch" wire:model='codeSearch'>
                    </div>
                    <div class="mb-3">
        ...
        <div class="col-span-2">
            ...
            {{-- Container --}}
            <div class="rounded-lg border border-[#fadde1] p-4 max-h-[90vh] overflow-y-scroll">
                {{-- Container Item --}}
                        ...
                        @foreach ($bookingData as $item)
                            ...
                            <div class="flex-auto border-b border-[#fadde1]">
                                    ...
                                    <div class="flex gap-4 ">
                                        <div class="">
                                            ...
                                            <p class="font-semibold">
                                                {{ Carbon\Carbon::parse($item->scheduleDateBook->date_schedule)->format('l , d F Y') }}
                                                |
                                                {{ Carbon\Carbon::parse($item->scheduleTimeBook->time)->format('h:i A') }}
                                            </p>
                                        </div>
                                        <div class="">
                                            <small for="">Total Payment</small>
                                            <p class="font-semibold">$
                                                {{ $item->total_price_booking - $item->deposit_price_booking }}</p>
                                        </div>
                                    </div>
                                    <div class="">
                                        <div class="py-2 lg:py-0">
                                            @if ($item->status == 1)
                                                @if ($item->confirm_payment == '0' && $item->is_deposit_paid == '0')
                                        ...
                                        <ul>
                                            @foreach ($item->detailService as $service)
                                                <li class="font-semibold">
                                                    ({{ $service->service->category->name_service_categori }})
                                                    {{ $service->name_service }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="">
                                        ...
                                        <ul>
                                                ...
                                                <li class="font-semibold">{{ $item->review->description_review}}</li>
                                ...
                                <div class="flex gap-3 mb-3">
                                    <div class="flex-auto">
                                        <a href="{{ route('user.reschedule_or_cancel', ['uuid' => $item->uuid]) }}"><button
                                                class="w-full btn-normal">More</button></a>
                                    </div>
                                    @if ($item->status == 'completed' && empty($item->review))
                                        <div class="flex-auto">
                                            <button class="w-full btn-normal" data-modal-target="review-modal" data-modal-toggle="review-modal" wire:click='setIdBooking("{{ $item->id }}")'>Leave Review</button>
                                        </div>
                                    @endif
    ...
    {{-- Modal Leave Review --}}
     {{-- Modal Zone --}}
     <x-pages.modal.modal id='review-modal' title="Leave Review"
     submitFunction='leaveReview()'>

     <div class="">
        <textarea name="" id="" cols="30" rows="10" class="w-full form-control" wire:model='reviewDescription'></textarea>
     </div>
